...
void Scroll(GLFWwindow* window, double xoffset, double yoffset) {
	...
	// Limitar o nível de zoom a um intervalo sensível, se desejar
	float minZoom = 0.1f;
	float maxZoom = 10.0f;
...
void MovePooltablle(GLFWwindow* window, double xpos, double ypos) {
	// Verificar se o mouse está sendo arrastado
	if (isMouseDragging) {
		// Calcular o movimento do mouse apenas no eixo x
...
void Animator()
{
	...
	if (animate && !animateStop) {
		// move a bola
		ballPos[ballToBeAnimated].x += 0.005f;
